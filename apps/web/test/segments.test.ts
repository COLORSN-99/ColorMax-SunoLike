...
test("S6-T7 reducer：failed 清除本轮 workflow 段（thinking/terminal/suno），error 保留 raw 于调试区", () => {
  ...
  assert.ok(err?.kind === "error" && err.raw === "HTTP 429 too many" && err.reviewStreaming === true);
  // error_review 终态填充（Step6 服务端接线后生效）
  segs = applyEvent(segs, { type: "error_review_delta", callId: "e", delta: "评审中…" });
  segs = applyEvent(segs, { type: "error_review", callId: "e", category: "quota", resolvableByCli: false, headline: "Suno 配额不足", steps: ["等待恢复"] });
  const err2 = segs[segs.length - 1];
  assert.ok(err2?.kind === "error" && err2.category === "quota" && err2.steps[0] === "等待恢复" && err2.reviewStreaming === false && err2.reviewText === "评审中…");
});
