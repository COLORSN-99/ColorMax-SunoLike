  ...
  */
  ...
  public async getCaptcha(): Promise<string | null> {
    if (!await this.captchaRequired())
      return null;
    // 二次开发: fail-fast —— 不启动浏览器/CAPTCHA 解算；交由 CookiePool 轮换或人工处理
    throw new CaptchaRequiredError("Suno CAPTCHA 触发: 请轮换 cookie 或人工验证");
  }
  ...
  private async generateSongs(
    ...
    if (wait_audio) {
      ...
      while (Date.now() - startTime < this.waitAudioMs) {
        ...
        await sleep(this.pollInterval() * 0.6, this.pollInterval() * 1.2);
  ...
  private pollInterval(): number {
    return Math.max(0.05, Math.min(5, this.waitAudioMs / 60_000));
  }
}
