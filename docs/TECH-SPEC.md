...
- 设置页字段（真实 API Key 接口，2026-09-01 多服务商+档案升级）：**服务商下拉（9 预置 + 自定义）/ API 格式 / BaseURL / API Key / Model（AutoComplete 推荐+自由填）/ maxTokens / 温度 / thinking**；选中服务商自动填充各家 base/格式/推荐模型，并给「获取 API Key / 查看余额·充值 / 接入文档」直达链接。**每服务商独立 profile（自定义按规范化 BaseURL 分桶）**：有历史 Key 仅显示 `sk-…末四位`，raw Key 永不回传浏览器；无历史 Key 留空；未输入 Key 保存=保留、输入新 Key=替换、显式「清除」才删除当前档案 Key。
