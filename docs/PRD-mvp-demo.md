...
2. 验收脚本 = 完整链路（输入→意图→规划→LangGraph 调度 Suno→回传→Leader 对齐评判→下载源格式音频）
...
| 阶段 | 交付 | 验收 |
|---|---|---|
| M1 | 骨架 + 设置面板 + LLM 真实调用（意图/规划 JSON） | 输入→规划可见（真实 LLM） |
| M2 | LangGraph 编排 + 对齐评判（+Mock 引擎调试链路） | 图跑通（含重派回环） |
| M3 | SunoAdapter 集成 + 交付页 | **完整验收链路通过** |
